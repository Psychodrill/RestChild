...
namespace RestChild.DocumentGeneration
{
    public static partial class WordProcessor
    {
        ...
        /// <summary>
        ///     Уведомление о предоставлении путёвки (1075.4)
        /// </summary>
        public static IDocument NotificationAboutTourAlternative(IUnitOfWork unitOfWork, Account account, long requestId)
        {
            var request = unitOfWork.GetById<Request>(requestId);
            ...
            using (var ms = new MemoryStream())
            {
                using (var wordDocument = WordprocessingDocument.Create(ms, WordprocessingDocumentType.Document))
                {
                    ...
                    doc.AppendChild(new Paragraph(
                        new ParagraphProperties(new Justification { Val = JustificationValues.Center }),
                        new Run(new RunProperties().SetFont().SetFontSize(Size28).Bold(),
                            new Text("Уведомление об осуществлении выбора варианта организации отдыха и оздоровления детей на основании бесплатной путевки для отдыха и оздоровления с датами заезда, начиная с 1 апреля по 27 июля 2020 г."))));
                    doc.AppendChild(new Paragraph(
                        new ParagraphProperties(new Justification { Val = JustificationValues.Center }),
                        new Run(new RunProperties().SetFont().SetFontSize(Size28).Bold(),
                ...
                {
                    FileBody = ms.ToArray(),
                    FileName = "Уведомление о выборе сертификата на отдых и оздоровление.docx",
                    MimeType = mimeType,
                    MimeTypeShort = extention
                };
            }
        ...
        /// <summary>
        ///     Уведомление о выборе отдыха в 2021 году
        /// </summary>
        public static IDocument NotificationAboutTourChoose(IUnitOfWork unitOfWork, Account account, long requestId)
        {
            var request = unitOfWork.GetById<Request>(requestId);
            ...
            using (var ms = new MemoryStream())
            {
                using (var wordDocument = WordprocessingDocument.Create(ms, WordprocessingDocumentType.Document))
                {
                    ...
                    SectionProperties sectionProperties1 = new SectionProperties() { RsidR = "00F44720", RsidSect = "00406F0B" };
                    PageSize pageSize1 = new PageSize() { Width = (UInt32Value)12240U, Height = (UInt32Value)15840U };
                    PageMargin pageMargin1 = new PageMargin() { Top = 567, Right = (UInt32Value)850U, Bottom = 709, Left = (UInt32Value)1134U, Header = (UInt32Value)708U, Footer = (UInt32Value)708U, Gutter = (UInt32Value)0U };
                    sectionProperties1.Append(pageSize1);
                    sectionProperties1.Append(pageMargin1);

                    var b = new Body();
                    b.Append(sectionProperties1);
                    ...
                    doc.AppendChild(new Paragraph(
                        new ParagraphProperties(new Justification { Val = JustificationValues.Center }),
                        new Run(new RunProperties().SetFont().SetFontSize(Size28).Bold(),
                            new Text("Уведомление о необходимости выбора конкретной организации отдыха и оздоровления на основании решения о предоставлении бесплатной путевки для отдыха и оздоровления в 2021 году взамен бесплатной путевки для отдыха и оздоровления с датами заезда начиная с 1 апреля по 27 июля 2020 г."))));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            ...
                            new Run(titleRequestRunPropertiesBold.CloneNode(true),
                                ...
                                { Space = SpaceProcessingModeValues.Preserve }),
                            new Run(titleRequestRunPropertiesItalic.CloneNode(true),
                                new Text($"{request.DateRequest:dd.MM.yyyy HH:mm}"))));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            new ParagraphProperties(new Justification { Val = JustificationValues.Left },
                                new SpacingBetweenLines { After = Size20 }),
                            new Run(titleRequestRunPropertiesBold.CloneNode(true),
                                new Text("Дата и время принятия решения о предоставлении бесплатной путевки для отдыха и оздоровления в 2021 году взамен бесплатной путевки для отдыха и оздоровления с датами заезда начиная с 1 апреля по 27 июля 2020 г.: ")
                                { Space = SpaceProcessingModeValues.Preserve }),
                            new Run(titleRequestRunPropertiesItalic.CloneNode(true),
                                new Text($"{request.DateRequest:dd.MM.yyyy HH:mm}"))));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            new ParagraphProperties(new Justification { Val = JustificationValues.Left },
                                new SpacingBetweenLines { After = Size20 }),
                            new Run(titleRequestRunPropertiesBold.CloneNode(true),
                                new Text("Приоритетное время отдыха в 2021 году: ") { Space = SpaceProcessingModeValues.Preserve }),
                            new Run(titleRequestRunPropertiesItalic.CloneNode(true),
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            new ParagraphProperties(new Justification { Val = JustificationValues.Left },
                                new SpacingBetweenLines { After = Size20 }),
                            new Run(titleRequestRunPropertiesBold.CloneNode(true),
                                new Text("Дополнительное время отдыха в 2021 году: ") { Space = SpaceProcessingModeValues.Preserve }),
                            new Run(titleRequestRunPropertiesItalic.CloneNode(true),
                                new Text(string.Join(", ", request.TimesOfRest?.OrderBy(ss => ss.Order).Select(ss => ss.TimeOfRest.Name).ToArray() ?? new string[0])))));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            new ParagraphProperties(new Justification { Val = JustificationValues.Left },
                                new SpacingBetweenLines { After = Size20 }),
                            new Run(titleRequestRunPropertiesBold.CloneNode(true),
                                new Text("Приоритетное направление отдыха в 2021 году: ") { Space = SpaceProcessingModeValues.Preserve }),
                            new Run(titleRequestRunPropertiesItalic.CloneNode(true),
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            new ParagraphProperties(new Justification { Val = JustificationValues.Left },
                                new SpacingBetweenLines { After = Size20 }),
                            new Run(titleRequestRunPropertiesBold.CloneNode(true),
                                new Text("Дополнительное направление отдыха в 2021 году: ") { Space = SpaceProcessingModeValues.Preserve }),
                            new Run(titleRequestRunPropertiesItalic.CloneNode(true),
                                new Text(string.Join(", ", request.PlacesOfRest?.OrderBy(ss => ss.Order).Select(ss => ss.PlaceOfRest.Name) ?? new string[0])))));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            new ParagraphProperties(new Justification { Val = JustificationValues.Both },
                                ...
                                new SpacingBetweenLines { After = Size20 }),
                            new Run(titleRequestRunProperties.CloneNode(true),
                                new Text("В период с 11 по 25 декабря 2020 г. включительно Вам необходимо повторно обратиться в офис ГАУК \"МОСГОРТУР\" для выбора конкретной организации отдыха и оздоровления. Выбор конкретной организации отдыха и оздоровления будет осуществляться из числа предлагаемых ГАУК \"МОСГОРТУР\" в соответствии с указанными Вами сведениями о приоритетном времени и направлении отдыха и оздоровления в 2021 году."){ Space = SpaceProcessingModeValues.Preserve })));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            new ParagraphProperties(new Justification { Val = JustificationValues.Both },
                                ...
                                new SpacingBetweenLines { After = Size20 }),
                            new Run(titleRequestRunProperties.CloneNode(true),
                                new Text("Обращаем Ваше внимание, что прием в офисе ГАУК \"МОСГОРТУР\" осуществляется исключительно по предварительной записи. Запись производится на свободную дату и время.") { Space = SpaceProcessingModeValues.Preserve })
                        ));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            new ParagraphProperties(new Justification {Val = JustificationValues.Both},
                                ...
                                new SpacingBetweenLines {After = Size20}),
                            new Run(titleRequestRunProperties.CloneNode(true),
                                new Text("В случае неучастия в период с 11 по 25 декабря 2020 г. включительно в выборе конкретной организации отдыха и оздоровления либо при отказе от всех предложенных вариантов будет сформирован отказ в предоставлении услуг отдыха и оздоровления.")
                                    {Space = SpaceProcessingModeValues.Preserve})
                        ));
                ...
                {
                    FileBody = ms.ToArray(),
                    FileName = "Уведомление о выборе варианта отдыха на 2021.docx",
                    MimeType = mimeType,
                    MimeTypeShort = extention
                };
            }
        ...
        public static IDocument NotificationDataSwitch(IUnitOfWork unitOfWork, Account account, long requestId)
        {
            ...
            if ((request.StatusId == (long)StatusEnum.Reject || request.StatusId == (long)StatusEnum.CertificateIssued) && request.DeclineReasonId == dr5)
            {
            ...
            if (request.StatusId == (long) StatusEnum.Reject && request.DeclineReasonId == (long)DeclineReasonEnum.RefuseAllVariantsCovid2020)
            {
                var doc = NotificationRefuse1080CovidUniversal(request, account);
                doc.RequestFileTypeId = (long)RequestFileTypeEnum.NotificationRefuse;
                return doc;
            }

...
namespace RestChild.DocumentGeneration
{
    public static partial class WordProcessor
    {
        ...
        /// </summary>
        private static IDocument NotificationRefuse108012(Request request, Account account)
        {
            var forMpguPortal = request.SourceId == (long) SourceEnum.Mpgu;

            if (forMpguPortal)
            {
                return PdfProcessor.NotificationRefuse108012(request);
            }
            ...
            using (var ms = new MemoryStream())
            {
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            ...
                            new Run(titleRequestRunPropertiesBold.CloneNode(true),
                                new Text("Основание отказа: ") {Space = SpaceProcessingModeValues.Preserve}),
                            new Run(titleRequestRunProperties.CloneNode(true),
                                new Text(FederalShort2020Law + " (далее – Порядок)."))));
                ...
                {
                    FileBody = ms.ToArray(),
                    FileName = "Уведомление об отказе от всех предложенных вариантов.docx",
                    MimeType = mimeType,
                    MimeTypeShort = extention
                };
            }
        ...
        /// <summary>
        ///     Уведомление формируемое при отказе от предложенных вариантов (подставляется причина отказа, только word)
        /// </summary>
        private static IDocument NotificationRefuse1080CovidUniversal(Request request, Account account)
        {
            using (var ms = new MemoryStream())
            {
                using (var wordDocument = WordprocessingDocument.Create(ms, WordprocessingDocumentType.Document))
        ...
        /// </summary>
        private static IDocument NotificationRefuse108013(Request request, Account account)
        {
            var forMpguPortal = request.SourceId == (long) SourceEnum.Mpgu;
            ...
            using (var ms = new MemoryStream())
            {
                using (var wordDocument = WordprocessingDocument.Create(ms, WordprocessingDocumentType.Document))
                {
                    ...
                    var simple = request.StatusId == (long) StatusEnum.CertificateIssued;
                    ...
                    doc.AppendChild(
                        ...
                        simple
                        ? new Run(new RunProperties().SetFont().SetFontSize(Size28).Bold(),
                            new Text("Уведомление о неучастии заявителя в выборе конкретной организации отдыха и оздоровления"))
                        : new Run(new RunProperties().SetFont().SetFontSize(Size28).Bold(),
                            new Text("Уведомление об отказе в предоставлении услуг отдыха и оздоровления"),
                            new Break(),
                            new Text("в связи с неучастием заявителя во втором этапе заявочной кампании"))));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            ...
                            new Run(titleRequestRunPropertiesItalic.CloneNode(true),
                                new Text(simple
                                    ? "Услуга оказана. Решение отрицательное."
                                    : "отказ в предоставлении услуг отдыха и оздоровления."))));
                    ...
                    doc.AppendChild(
                        new Paragraph(
                            ...
                            simple
                            ? new Run(titleRequestRunProperties.CloneNode(true),
                                        new Text("пункт 8.8. Порядка организации отдыха и оздоровления детей, находящихся"),
                                        new Break(),
                                        new Text("в трудной жизненной ситуации, утвержденного постановлением Правительства Москвы"),
                                        new Break(),
                                        new Text("от 22 февраля 2017 г. № 56-ПП \"Об организации отдыха и оздоровления детей, находящихся"),
                                        new Break(),
                                        new Text("в трудной жизненной ситуации\""
                                                 + " (далее – Порядок)."))
                            : new Run(titleRequestRunProperties.CloneNode(true),
                                        new Text("пункты 8.9. и 9.1.7 Порядка организации отдыха и оздоровления детей, находящихся в трудной жизненной ситуации, утвержденного постановлением Правительства Москвы от 22 февраля 2017 г. № 56-ПП \"Об организации отдыха и оздоровления детей, находящихся в трудной жизненной ситуации\""
                                         + " (далее – Порядок)."))));
                ...
                {
                    FileBody = ms.ToArray(),
                    FileName = "Уведомление в связи с неучастием.docx",
                    MimeType = mimeType,
                    MimeTypeShort = extention
                };
            }

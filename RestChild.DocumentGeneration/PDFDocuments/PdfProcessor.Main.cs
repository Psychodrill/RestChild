        ...
        internal static IDocument NotificationRefuse10805(IUnitOfWork unitOfWork, Request request, ICollection<long> years)
        {
            ...
            using (var ms = new MemoryStream())
            {
                using (var document = new Document(PageSize.A4, 60, 40, 40, 40))
                {
                    ...
                    PdfAddParagraph(document, 0, 1, Element.ALIGN_JUSTIFIED, 0,
                        new Chunk("Результат рассмотрения заявления: ", HeaderFont),
                        new Chunk($"учитывая получение услуг отдыха и оздоровления в предыдущие годы и исходя из даты и времени подачи заявления, оказание услуг отдыха и оздоровления в {request.TimeOfRest.Year} году не представляется возможным.", MainText));
            ...
            using (var ms = new MemoryStream())
            {
                using (var document = new Document(PageSize.A4, 60, 40, 40, 40))
                {
                    ...
                    PdfAddParagraph(document, 0, 1, Element.ALIGN_JUSTIFIED, firstLine_small,
                        ...
                        new Chunk(" Вам необходимо явиться в офис ГАУК \"МОСГОРТУР\" по адресу: г. Москва, Малый Харитоньевский переулок д. 6 стр. 3.", MainText));

...
namespace UtilsSmev.SmevServiceEncoder
{
	public class SmevServiceMessageEncoder : MessageEncoder
	{
		...
		private string soapEnvelope = "<s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\"><s:Header></s:Header><s:Body></s:Body></s:Envelope>";
      ...
      private ILoggingSmevRequest Logger { get; set; }
		...
		public override Message ReadMessage(ArraySegment<byte> buffer, BufferManager bufferManager, string contentType)
		{
			...
			// Проверяем подпись
			if (!(Signer.CheckSignature(xmlDocument, _storeLocation) || Signer.CheckSignature(xmlDocument2, _storeLocation)))
			{
		...
		public override ArraySegment<byte> WriteMessage(Message message, int maxMessageSize, BufferManager bufferManager, int messageOffset)
		{
			...
			var w = XmlWriter.Create(memoryStream, new XmlWriterSettings { OmitXmlDeclaration = true, Encoding = new UTF8Encoding(false) });
			xmlDocument.Save(w);
		...
		private void LogMessage(XmlDocument doc, bool input)
		{
			try
			{
				...
				else
				{
					...
					if (Logger != null)
						Logger.SaveMessage(input, doc.InnerXml.Replace("<?xml version=\"1.0\" encoding=\"utf-8\"?>", ""), s);
				}
			}
			catch
			{

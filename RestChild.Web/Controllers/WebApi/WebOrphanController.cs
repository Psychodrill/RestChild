...
namespace RestChild.Web.Controllers.WebApi
{
    ...
    public class WebOrphanController : BaseController
    {
        ...
        ///     Поиск детских домов
        /// </summary>
        internal CommonPagedList<OrphanageResultListModel> GetOrphanage(OrphanageFilterModel filter, int pageSize = -1, bool forExcel = false)
        {
            if (pageSize < 0)
            {
            ...
            if (!string.IsNullOrWhiteSpace(filter.Address))
            {
                query = query.Where(ss =>
                    ss.OrphanageOrganizationAddresses.Any(sx => sx.Address.Name.ToLower().Contains(filter.Address.ToLower())));
            }
            ...
            if (forExcel)
            {
                entity = query.ToList().Select(ss => new OrphanageResultListModel
                {
                    ...
                    Phone = ss.Phone,
                    EMail = ss.Email,
                    FioRfr = string.Join("\n\r", ss.OrganisatonCollaborators?.Where(oc => oc.EntityId == null && oc.PositionId == (long)OrphanageCollaboratorType.ResponsibleForRest).OrderBy(ox => ox.Id).Select(oc => oc.Applicant.GetFio()).ToList()),
                    EMailRfr = string.Join("\n\r", ss.OrganisatonCollaborators?.Where(oc => oc.EntityId == null && oc.PositionId == (long)OrphanageCollaboratorType.ResponsibleForRest).OrderBy(ox => ox.Id).Select(oc => oc.Applicant.Email).ToList()),
                    PhoneRfr = string.Join("\n\r", ss.OrganisatonCollaborators?.Where(oc => oc.EntityId == null && oc.PositionId == (long)OrphanageCollaboratorType.ResponsibleForRest).OrderBy(ox => ox.Id).Select(oc => {
            ...
            return new CommonPagedList<OrphanageResultListModel>(entity, pageNumber, pageSize > 0 ? pageSize : totalCount, totalCount);
        }

        /// <summary>
        ///     Поиск сотрудников детских домов
        /// </summary>
        internal CommonPagedList<OrphanageCollaboratorsResultListModel> GetOrphanageCollaborators(OrphanageCollaboratorsFilterModel filter)
        {
        ...
        internal CommonPagedList<OrphanagePupilsResultListModel> GetPupils(OrphanagePupilsFilterModel filter)
        {
            ...
            var query = UnitOfWork.GetSet<Pupil>().Where(ss => ss.OrphanageAddress.OrganisationId == filter.OrphanageId && (ss.Child.EntityId == null || ss.Child.EntityId == ss.Child.Id)).AsQueryable();
            ...
            if (!string.IsNullOrWhiteSpace(filter.Name))
            {
                ...
                {
                    query = query.Where(ss => ss.Child.FirstName.ToLower().Contains(n.ToLower()) || ss.Child.LastName.ToLower().Contains(n.ToLower()) || ss.Child.MiddleName.ToLower().Contains(n.ToLower()));
                }
            }
        ...
        internal void FillGroups(OrphanageGroupsFilterModel filter, int pageSize = -1)
        {
            ...
            filter.VacationPeriods = UnitOfWork.GetSet<PupilGroupVacationPeriod>().Where(ss => !ss.IsDeleted || ss.Id == filter.VacationPeriod)
                .ToDictionary(ss => ss.Id, sx => sx.Name);
        }
        ...
        internal void FillGroup(OrphanageGroupModel model)
        {
            ...
            model.FormsOfRest = UnitOfWork.GetSet<FormOfRest>().Where(ss => !ss.IsDeleted || ss.Id == model.FormOfRestId).ToDictionary(ss => ss.Id.ToString(), sx => sx.Name);
            model.VacationPeriods = UnitOfWork.GetSet<PupilGroupVacationPeriod>().Where(ss => !ss.IsDeleted || ss.Id == model.FormOfRestId).ToDictionary(ss => ss.Id.ToString(), sx => sx.Name);
        }
        ...
        /// </summary>
        internal IQueryable<ListOfChilds> PrepareQueryPupilGroupList(OrphanagePupilGroupListFilterModel filter, int pageSize = -1)
        {
            var query = UnitOfWork.GetSet<ListOfChilds>().Where(ss => ss.StateId != StateMachineStateEnum.PupilGroupList.Deleted && ss.PupilGroupRequest.Any()).AsQueryable();
            ...
            if (!filter.OrphanageId.HasValue)
            {
                ...
                if (orgs.Length > 0)
                {
                    query = query.Where(c => c.PupilGroupRequest.Any(sx => orgs.Contains(sx.PupilGroup.OrganizationId)));
                }
            }
            else
            {
                query = query.Where(ss => ss.PupilGroupRequest.Any(sx => sx.PupilGroup.OrganizationId == filter.OrphanageId));
            }
            ...
            if (!string.IsNullOrWhiteSpace(filter.Name))
            {
                ...
                foreach (var name in nameArr.Where(ss => !string.IsNullOrWhiteSpace(ss)))
                {
                    query = query.Where(ss => ss.PupilGroupRequest.Any(sx => sx.PupilGroup.Name.ToLower().Contains(name.ToLower())));
                }
            }

            if (filter.YearOfRest.HasValue && filter.YearOfRest.Value > 0)
            {
                query = query.Where(ss => ss.PupilGroupRequest.Any(sx => sx.PupilGroup.YearOfRestId == filter.YearOfRest));
            }

            if (filter.FormOfRestId.HasValue && filter.FormOfRestId.Value > 0)
            {
                query = query.Where(ss => ss.PupilGroupRequest.Any(sx => sx.PupilGroup.FormOfRestId == filter.FormOfRestId));
            }
        ...
        public IList<BaseResponse> GetPupilGroupTimesOfRest(long orphanagePupilGroupId)
        {
            ...
            var res =
                UnitOfWork.GetSet<TimeOfRest>()
                    .Where(t => t.IsActive && t.YearOfRestId == group.YearOfRestId && t.TypeOfRestId == group.FormOfRest.TypeOfRestId)
                    .OrderBy(p => p.Name)
        ...
        {
            var orgs = UnitOfWork.GetSet<Organization>().Where(ss => !ss.IsDeleted && ss.Orphanage == true).AsQueryable();

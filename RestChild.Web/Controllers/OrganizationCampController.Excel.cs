...
{
	public partial class OrganizationCampController
    {
		...
		/// выгрузка сведений
		/// </summary>
		public ActionResult ExcelOrganizationCampList(OrganizationCampSearchModel model)
		{
            ...
            var fileName = "Справочник лагерей РФ";
		...
		/// выгрузка по списка
		/// </summary>
		protected void Sheet(ExcelWorksheet sheet, OrganizationCampSearchModel model)
		{
			SetUnitOfWorkInRefClass(UnitOfWork);
            var pageSize = 5000;
            ...
            var entity = GetOrganizationCampQuery(model);
            ...
            var list = entity.OrderBy(i => i.Id)
                ...
                .Take(pageSize)
                .ToArray();
            model = model ?? new OrganizationCampSearchModel();
            FillCollections(model, UnitOfWork);
            var row = 1;
            ...
            sheet.Name = "Справочник лагерей РФ";
            var col = 1;

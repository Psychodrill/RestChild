...
{
	public partial class OrganizationGBUController
    {
		...
		/// выгрузка сведений
		/// </summary>
		public ActionResult ExcelOrganizationGBUList(OrganizationGBUSearchModel model)
		{
		...
		/// выгрузка по списка
		/// </summary>
		protected void Sheet(ExcelWorksheet sheet, OrganizationGBUSearchModel model)
		{
			SetUnitOfWorkInRefClass(UnitOfWork);
            var pageSize = 5000;
            ...
            var entity = GetOrganizationGBUQuery(model);
            ...
            var row = 1;
			var cols = new[] { "№", "Название ГБУ", "Сокращенное название ГБУ", "Регион"};
			...
			foreach (var hotel in entity)
			{
              ...
              //  SetValue(sheet, $"{hotel.Region.Name}", row, 4);
                SetValue(sheet, $"{hotel.FactAddress}", row, 4);

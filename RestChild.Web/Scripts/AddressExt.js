...
addressControlClear = function () {
};
// Метод динамической установки значения адреса
addressControlSetValue = function (val, needVladenie) {
    if (needVladenie === void 0) { needVladenie = true; }
    if (!val) {
        val = { id: 0 };
    }
    ...
    $('.StreetNotFoundCheckbox').change(function (e) {
...
addressControlGetValue = function () {
    ...
    if ($(".StreetNotFoundCheckbox").prop('checked')) { // $('.StreetNotFoundCheckbox-h').val() === 'True' ???
        ...
        if (exists) {
            var data = $("input.street-autocompleteAR").select2("data");
            if (data) {
                fiasId = data.id;
                street = data.text;

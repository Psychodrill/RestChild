$(function () {
    ...
    $('#Data_TypeOfServiceId').change(function (e) {
        $.ajax({
    ...
    function processHotelTable() {
        if ($('#hotelsTable>tbody').children('tr').length > 0) {
            $('#hotelsTable>tbody>tr').each(function (num, val) {
                $(val).find('input[name^="Hotels["],select[name^="Hotels["]').each(function (inputNum, input) {
    ...
    function processServiceTable() {
        ...
        if ($('#prices>tbody').children('tr').length > 0) {
            $('#prices').show();
            $('#prices>tbody>tr').each(function (num, val) {
                $(val).find('input[name^="Prices["],select[name^="Prices["],textarea[name^="Prices["]').each(function (inputNum, input) {
    ...
    $('#hotelsTable').on('click', '.remove-hotel-btn', function (e) {
        var row = $(e.target).closest('tr');
        row.remove();
        processHotelTable();
    });
    ...
    $('#addonServiceForm').submit(function () {
...
function confirmStateButtonAddonService(formSelector, actionSelector, actionCode, buttonName, description, commentSelector) {
    ...
    $('#prices').children('tbody').children('tr').each(function (i, e) {
        var $e = $(e);
...
//# sourceMappingURL=AddonServiceEdit.js.map
declare var MilitaryDutyReservist: string;
declare var TypeOfEducationHigh: string;

$(() => {
	$('.datepicker-anytime').datetimepicker({ showTodayButton: true, format: 'DD.MM.YYYY' });

	function setCollectionNum(pattern: string, container = 'tr') {
		$(container + ':has(input[name^="' + pattern + '"])').each((trNum, tr) => {
			$(tr).find('input[name^="' + pattern + '"]').each((inputNum, input) => {
				var $input = $(input);
	 ...
	 function setupTableCollection(addButtonSelector : string, templateSelector : string, tableSelector : string, inputNamePattern: string, removeButtonSelector : string) {
		 ...
		 $(document).on('click', removeButtonSelector, (e) => {
	...
	$('.date').inputmask("d.m.y", {
	 ...
	 $('#Data_DocumentTypeId').change(() => {
		 onDocChange();
	 });
	 onDocChange();

	 $('#Data_HaveMiddleName').change(() => {
	...
	$('.counselor-skill-checkbox').change((e) => {
		if ($(e.target).is(':checked')) {

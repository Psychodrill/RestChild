...
$(() => {
	...
	$(document).on('click', 'input.childs-payed', (e) => {
		var $target = $(e.target);
	...
	$(document).on('click', 'input.attendants-payed', (e) => {
	...
	$(document).on('click', 'input.list-paid', (e) => {
		var target = $(e.target);
	...
	$('select').select2();
	$('#Data_TypeOfRestId').select2().on('change', (e) => {
	...
	function prepareRateTable() {
		if ($('#roomRateTable tbody').children('tr').length > 0) {
			$('#roomRateTable').show();
			$('#roomRateI').hide();
			$('#roomRateTable tbody tr').each((num, val) => {
				$(val).find('input[name^="RoomRates["]').each((inputNum, input) => {
	...
	function removeRoomrate(e: JQueryEventObject) {
	...
	$('body').on('click', '.typeOfRoomChoose', (e) => {
	...
	$('body').on('click', '.removeTourVolume', (e) => {
	...
	$('body').on('click', '.btn-childlist-select', (e) => {
	...
	$('body').on('click', '.childlist-exclude', (e) => {
		BootstrapDialog.show({
			title: 'Подтвердить действие',
	...
	$('body').on('click', '.childlist-expand', (e) => {
		...
		if (!childTr.hasClass('loaded')) {
			...
			$.ajax({
				...
				if (received.childs != null && received.childs.length > 0) {
					childs = $.map(received.childs, (val, i) => {
						return {
							num: i + 1,
				...
				if (received.attendants != null && received.attendants.length > 0) {
					attendants = $.map(received.attendants, (val, i) => {
						return {
							num: i + 1,
							lastName: val.lastName != null ? val.lastName : '-',
							firstName: val.firstName != null ? val.firstName : '',
	...
	$('#ChildListsAddFilterSearch').click(() => {
		$('#ChildListsAddBody').html('<div class="text-center">Получение данных <img class="text-center" width="20" height="20" src="/Content/images/spinner.gif"/> </div>');
	...
	$('#ChildListsAddFilterClear').click(() => {
		$('#ChildListsAddFilterVedomstvo').select2('val', { id: '', text: '-- Не выбрано --' });
		$('#ChildListsAddFilterChildsFrom').val(null);
		$('#ChildListsAddFilterChildsTo').val(null);
		$('#ChildListsAddBody').html('<div class="text-center">Получение данных <img class="text-center" width="20" height="20" src="/Content/images/spinner.gif"/> </div>');
	...
	function fixPartyNums() {
		$('.party').each((partyNum, party) => {
			$(party).find('input[name^="Data.Partys["]').each((inputNum, input) => {
			...
			$(party).find('.child-in-party-table tbody tr').each((trNum, tr) => {
				$(tr).find('input[name^="Data.Partys["]').each((inputNum, input) => {
	...
	$('body').on('click', '.btn-add-child-in-party', (e) => {
	...
	$('body').on('click', '.child-in-party-remove', (e) => {
		var party = $(e.target).closest('.party');
	...
	$('.btn-add-party').click(() => {
		...
		$('.children-not-in-parties').each((num, val) => {
			newPartyChildSelect.append(optionFn({
			...
			buttons: [
				{
					...
					action: (dialogItself) => {
						var party = $(e.target).closest('.party');
						party.find('.child-in-party-id').each((num, val) => {
	...
	$('#Data_ContractId').select2({
		...
		ajax: {
			...
			data: (term, page) => {
				...
				return {
					signNumber: term,
					organizationId: null, //limitOnVedomstvo != null && limitOnVedomstvo.vedomstvoId != null ? limitOnVedomstvo.vedomstvoId : '',
					yearOfRestId: null,
					onTransport: null,
					onRest: null
				};
...
function initTypeOfSubrestrtiction() {
   let $restriction = $('.restriction-select');
   let $subres = $('.subrestriction-select');
   let $div = $('.type-of-subrestriction');

...
var ParentsNeed = /** @class */ (function () {
    function ParentsNeed() {
    }
    return ParentsNeed;
}());
...
$(function () {
    ...
    $('.transport-info-fio, .camper-name').each(function (index, element) {
        setTimeout(function () {
            var parentsJs = getParents(element);
            var parentIndexs = [];
            parentsJs.forEach(function (parentElement, i) {
            ...
            result.push({
                searchString: element.innerHTML.toLowerCase(),
    ...
    function searchPerson(e) {
        ...
        setTimeout(function () {
            ...
            if (currentSearchString !== curVal && item === curVal) {
                ...
                if (item === curVal) {
                    currentSearchString = curVal;
                    itemToHide.filter(onlyUnique).forEach(function (el) {
                        setTimeout(function () {
                            if (itemToShow.indexOf(el) < 0) {
                                var i = parentsList[el];
                                if (i && i.visibled && !i.panelCollapse) {
                                    i.item.hide();
                                    i.visibled = false;
                                }
                    ...
                    itemToShow.filter(onlyUnique).forEach(function (el) {
                        setTimeout(function () {
                            var i = parentsList[el];
                            if (i && !i.visibled && !i.panelCollapse) {
                                i.item.show();
    ...
    $('#DirectoryFlightButtonForAll').click(function () {
        ...
        if (selected.id) {
            $('select.transport-info-directory-flight-select').not('[disabled="disabled"]').each(function (i, e) {
                var item = $(e);
    ...
    $('.directory-flight-set-button').click(function (e) {
    ...
    //});
    $('select.transport-info-directory-flight-select-for-hotels').change(function (e) {
    ...
    $('#transportForm').submit(function () {
        if (typeof (Storage) !== 'undefined') {
            ...
            window.sessionStorage['TransportInfo_CampsOpen'] = JSON.stringify($('.camp-accordion.in').map(function (i, val) { return $(val).attr('data-camp-id'); }).get());
            window.sessionStorage['TransportInfo_HotelsOpen'] = JSON.stringify($('.hotel-accordion.in').map(function (i, val) { return $(val).attr('data-hotel-id'); }).get());
            window.sessionStorage['TransportInfo_PartiesOpen'] = JSON.stringify($('.party-accordion.in').map(function (i, val) { return $(val).attr('data-party-id'); }).get());
            window.sessionStorage['TransportInfo_RequestsOpen'] = JSON.stringify($('.request-accordion.in').map(function (i, val) { return $(val).attr('data-request-id'); }).get());
        }
        return true;
    });
...
function overrideConfirmStateButton(formSelector, actionSelector, actionCode, buttonName, description, commentSelector) {
    if (actionCode === formActionString) {
        ...
        if (emptyWagonInputs.length !== 0 || emptyPlaceNumberInputs.length !== 0 || emptyFlightSelects.length !== 0 || emptyDepartureDates.length !== 0) {
            ...
            emptyWagonInputs.each(function (num, val) {
                $(val).closest('.form-group').addClass('has-error');
            });
            emptyPlaceNumberInputs.each(function (num, val) {
                $(val).closest('.form-group').addClass('has-error');
            });
            emptyFlightSelects.each(function (num, val) {
                $(val).closest('.form-group').addClass('has-error');
            });
            emptyDepartureDates.each(function (num, val) {
            ...
            $('.departure-date').each(function (num, val) {
...
//# sourceMappingURL=TransportEdit.js.map
$(function () {
    var template = $(".OrphanageAddress").first().get(0).outerHTML;
    setStreetAR($(".street-autocompleteAR").not("inited").addClass("inited"));
    InitSelects();
    $(".OrphanageFormSave").click(function (e) {
        $(e.target).prop("disabled", true);
        $("#OrphanageForm").submit();
    });
    $(".address-add").click(function () {
        var OrphanageAddressIds = $(".OrphanageAddressIndex").map(function () { return $(this).val(); }).get();
        var newOrphanageAddressId = Math.max.apply(Math, OrphanageAddressIds) + 1;
        var block = $(template);
    ...
    $("#OrphanageForm").on("click", ".address-remove", function () {
    ...
    function setStreetAR(element) {
        element.select2({
            ...
            var block = $(this).closest(".OrphanageAddress");
...
//# sourceMappingURL=OrphanageEdit.js.map
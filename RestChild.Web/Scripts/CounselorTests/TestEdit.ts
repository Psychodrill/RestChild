$(() => {
	function fixTableNums() {
		...
		rows.each((partyNum, party) => {
			$(party).find('input[name^="Questions["],textarea[name^="Questions["],select[name^="Questions["]').each((inputNum, input) => {
			...
			$(party).find('input[type="radio"]').each((i, r) => {
				$(r).attr('name', 'group_' + partyNum);
			});

			var answers = $(party).find('.answers tbody tr');
			answers.each((trNum, tr) => {
				$(tr).find('input[name^="Questions["]').each((inputNum, input) => {
	...
	function fixSubjectTableNums() {
		var rows = $('#tblSubject>tbody>tr');
		rows.each((partyNum, party) => {
			$(party).find('input[name^="Subjects["],textarea[name^="Subjects["]').each((inputNum, input) => {
	...
	$('#add-subject, #add-subject-bottom').click((e) => {
	...
	$('#tblSubject').on('click', '.remove-subject', (e) => {
	...
	$('#add-question, #add-question-bottom').click((e) => {
		$('#tblQuestions>tbody').append($($('#rowQuestion').html()));
		fixTableNums();
		refreshSubject();
	});

	$('#tblQuestions').on('click', '.add-answer', (e) => {
	...
	$('#tblQuestions').on('click', '.remove-answer', (e) => {
		$(e.target).parent().parent().remove();
		fixTableNums();
		refreshSubject();
	});
	$('#tblQuestions').on('click', '.remove-question', (e) => {
		$(e.target).parent().parent().remove();
		fixTableNums();
	});

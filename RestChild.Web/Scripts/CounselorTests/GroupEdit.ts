$(() => {
	...
	function fixTableNums() {
		$('#tbl-add-conselor>tbody>tr').each((partyNum, party) => {
			$(party).find('input[name^="Results["],textarea[name^="Results["]').each((inputNum, input) => {
		...
		$('#tbl-add-test>tbody>tr').each((partyNum, party) => {
			$(party).find('input[name^="Tests["],textarea[name^="Tests["]').each((inputNum, input) => {
	...
	$('#add-conselor, #add-conselor-bottom').click((e) => {
	...
	$('#add-test, #add-test-bottom').click((e) => {
	...
	$('#tbl-add-conselor').on('click', '.remove-counselor', (e) => {
		$(e.target).parent().parent().remove();
		fixTableNums();
	});

	$('#tbl-add-test').on('click', '.remove-test', (e) => {
		$(e.target).parent().parent().remove();
		fixTableNums();
	});
	...
	$('#tbl-add-test').on('change', '.count-attempt', (e) => {

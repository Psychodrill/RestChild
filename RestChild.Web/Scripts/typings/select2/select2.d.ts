...
interface JQuery {
    ...
    select2(): JQuery;
	select2(p1: { initSelection: (element: JQuery, callback: (data: any) => void) => void;minimumInputLength: number;ajax: { url: string;dataType: string;type: string;contentType: string;params: { contentType: string };data: (term: string, page: number, element: any) => string;results: (data: any, page: number) => { results };cache: boolean } });
